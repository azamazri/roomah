-- ============================================================================
-- MIGRATION 14 FIXED: Admin Tables (No Foreign Keys - Safe Version)
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.cv_verification_queue (
  ...
  created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (cv_user_id) REFERENCES public.cv_data(user_id) ON DELETE CASCADE,
  FOREIGN KEY (user_id) REFERENCES public.profiles(user_id) ON DELETE CASCADE
);

-- Taaruf Session Reports
...
-- Social Media Settings Table (platform-wide social media config)
...
-- Analytics/Dashboard Metrics (for caching dashboard data)
CREATE TABLE IF NOT EXISTS public.dashboard_metrics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  metric_type VARCHAR(100) NOT NULL, -- user_signups, cv_approvals, taaruf_matches, etc
...
CREATE INDEX idx_admin_platform_settings_key ON public.admin_platform_settings(setting_key);
CREATE INDEX idx_admin_feature_flags_name ON public.admin_feature_flags(flag_name);
CREATE INDEX idx_admin_feature_flags_enabled ON public.admin_feature_flags(enabled);
CREATE INDEX idx_social_media_accounts_user ON public.social_media_accounts(user_id);
CREATE INDEX idx_social_media_accounts_platform ON public.social_media_accounts(platform);
CREATE INDEX idx_social_media_accounts_verified ON public.social_media_accounts(is_verified);
CREATE INDEX idx_audit_logs_actor ON public.audit_logs(actor_id);
CREATE INDEX idx_audit_logs_action ON public.audit_logs(action);
CREATE INDEX idx_audit_logs_created ON public.audit_logs(created_at DESC);
CREATE INDEX idx_audit_logs_entity ON public.audit_logs(entity_type, entity_id);
CREATE INDEX idx_cv_verification_queue_status ON public.cv_verification_queue(status);
CREATE INDEX idx_cv_verification_queue_user ON public.cv_verification_queue(user_id);
CREATE INDEX idx_cv_verification_queue_priority ON public.cv_verification_queue(priority DESC);
CREATE INDEX idx_taaruf_session_reports_status ON public.taaruf_session_reports(status);
CREATE INDEX idx_taaruf_session_reports_session ON public.taaruf_session_reports(session_id);
...
CREATE INDEX idx_user_suspensions_dates ON public.user_suspensions(suspended_from, suspended_until);
...
CREATE INDEX idx_dashboard_metrics_type_date ON public.dashboard_metrics(metric_type, metric_date DESC);
