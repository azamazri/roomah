...
USING (enabled = TRUE OR is_admin(auth.uid()));
...
-- Only admins can view audit logs (allowing both SELECT and INSERT for system operations)
ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "admin_only_audit_logs_select"
ON public.audit_logs FOR SELECT
...
WITH CHECK (is_admin(auth.uid()) AND suspended_by = auth.uid());
...
WITH CHECK (is_admin(auth.uid()) AND initiated_by = auth.uid());
